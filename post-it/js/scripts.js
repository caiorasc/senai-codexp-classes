...
function atualizarSecao(secao) {
    ...
    for (var posicao = 0; posicao < nota.listaInterna.length; posicao++) {
        if (nota.listaInterna[posicao].editando) {
            //criar variavel para guardar o html de todas as nota.listaInterna que estão aparecendo na tela
            conteudoSecao += '<form class="note">' +
                '<input class="note__title" type="text" name="titulo" value="' + nota.listaInterna[posicao].titulo + '" placeholder="Título">' +
                '<textarea class="note__body" name="texto" rows="5" placeholder="Criar uma nota...">' + nota.listaInterna[posicao].texto + '</textarea>' +
            ...
            conteudoSecao += '<form class="note" onclick="editarFormulario(' + posicao + ', this.parentElement)">' +
                ...
                '</button>' +
                '<h1 class="note__title">' + nota.listaInterna[posicao].titulo + '</h1>' +
                '<p class="note__body">' + nota.listaInterna[posicao].texto + '</p>' +
                '</form>';
        }
    }
